...
- 服务端登出与授权确认（配合 new-api `prompt=consent` 确认页）：
  - 退出登录现在会调用 canvas 服务端 `POST /api/auth/logout`，清除该账号的 WanInter 绑定（access_token 与缓存密钥），不再仅清本地。
  - 退出后再次点击 WanInter 登录，应进入 new-api 的「授权确认」页（显示账号、可「同意并继续」或「取消」），而不是直接静默登回；点「取消」会回到 canvas 登录页并提示已取消。
- 云端渠道「刷新密钥」按钮：WanInter 账号在配置弹窗可手动刷新密钥列表与可用模型，在 new-api 后台新建/删除密钥后无需重新登录即可看到。
