...
func New() *gin.Engine {
	...
	api.GET("/auth/waninter/quota", middleware.UserAuth, gin.WrapF(handler.WanInterQuota))
	api.GET("/auth/waninter/keys", middleware.UserAuth, gin.WrapF(handler.WanInterKeys))
	api.POST("/auth/waninter/select-key", middleware.UserAuth, gin.WrapF(handler.WanInterSelectKey))
